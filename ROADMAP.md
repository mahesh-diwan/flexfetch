...
Ratatui is for _interactive_ TUI apps that own the terminal (raw mode, redraw loops); a
...
- Ratatui 4-step wizard (`flexfetch-cli/src/wizard.rs`, behind the `live`
  ...
  preset), 2) theme picker with live preview (theme::resolve + gradient), 3) layout (box style + frame), 4) save summary → writes
...
| 4.6  | **Filesystem deep dive**: BTRFS profile/compression/subvols, ZFS pools, ZRAM ratio, LVM mapping, LUKS cipher, optional SMART, mount options                                                                    | `fs-btrfs`, `fs-zfs`, `fs-zram`, `fs-lvm`, `fs-luks`, `fs-smart` | ⚠️ (partial) — `fsdeep.rs` ships (ZRAM ratio, LUKS devices, LVM volumes); BTRFS/ZFS/SMART depth pending |
...
| Task | What                                                                                                                                                            | Feature gate         | Status                                                                    |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | ------------------------------------------------------------------------- |
| 4.9  | **Diff mode** (`--diff <host1> <host2>`): compare local/remote/JSON datasets, 3-column aligned table, semantic highlight, HTML/MD export                        | `diff` (default)     | ✅ (Aug 2026) — `--diff <t1> <t2>`, `render_diff` + `resolve_diff_target` |
| 4.10 | **Infrastructure exports**: `--format ansible                                                                                                                   | terraform            | csv                                                                       | prometheus`+`--discover` mDNS service discovery | `export-infra` (default) | ✅ (Aug 2026) — `export_ansible`/`export_terraform`/`export_csv`/`export_prometheus`, `--format ansible\|terraform\|csv\|prometheus` |
...
| 4.14 | **Weather & geolocation**: embedded GeoLite2 + MET Norway API over a hand-rolled `TcpStream` HTTP/1.1 parser (no reqwest/hyper), 10-min cache | `weather`         | 🟡 (partial) — `weather` module ships (`modules/weather.rs`, wttr.in does geolocation); GeoLite2/MET-Norway-specific work remains |
| 4.15 | **Container deep introspection**: docker.sock via hyperlocal, Podman, Kubernetes pod/node/limits                                              | `container-deep`  | ⬜ — basic `container` detection ships (`modules/container.rs`); docker.sock/K8s deep work remains                                |
...
```
Phase 0-3 (done) ──► Pillar A (4.1-4.3 ✅) ──► Pillar B (4.4-4.8)
...
**Week 7** 4.13+4.16+4.17 · **Week 8** v2.0 release. Weeks 1, 2, 5, 6 ✅ landed (Aug 2026):
Pillar A (4.1–4.3) and Pillar C (4.9–4.12) complete. Remaining: Pillar B (4.4–4.8),
Pillar D (4.13–4.15, 4.14/4.15 partial) and Pillar E (4.16 asset compression, 4.17).

**Start here: Task 4.1** ✅ (done Aug 2026 — see below; it forced zero-allocation
collectors, which feed the live dashboard and the whole premium feel). Next work:
Pillar B (4.4–4.8) and 4.16 asset compression.
...
The remaining 686 ms is _network latency_, not spawn cost: publicip ~500 ms (api.ipify.org
...
- **wifi** (`wifi.rs`) — `nmcli … dev wifi list --rescan no` (space-separated! `=no` is
  rejected) uses NetworkManager's _cached_ scan: 24 ms vs 4.17 s. Caveat: on a fresh
...
| #    | Task                                                                                                                                                                                                                                                                                                                               | Effort  | Status                                                                                                                                                                                                                                              |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 7.9  | **Add `parallel` (rayon) to the release feature set** — the shipped binary currently collects modules _sequentially_ (`release.yml` uses `live,image-logos,completions`, no `parallel`). Conscious speed↔size tradeoff: rayon was excluded by the 0.2 diet (~2 MB release); re-adding costs ~100–200 KB, still under the 4 MB gate | Trivial | ✅ (Aug 2026)                                                                                                                                                                                                                                       |
...
features still marked ⬜ (4.4-4.8/4.13-4.16 — all behind off-by-default
...
6. Remaining optional/deferred: `clap_mangen` man-page regeneration, `notify`-
   ...
   re-run pending).
7. **Phase 4 — Domination — 🟡 4.1/4.2/4.3/4.9/4.10/4.11/4.12 done, 4.14/4.15 partial, 4.17 partial (Aug 2026)** —
